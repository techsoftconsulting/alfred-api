...
class RestaurantVerifyResetPasswordCodeDto {
  @ApiProperty()
  email: string;
  @ApiProperty()
  code: string;
...
class RestaurantRequestChangePasswordDto {
  @ApiProperty()
  email: string;
}

class RestaurantChangePasswordDto {
  @ApiProperty()
  email: string;
...
export class RestaurantAuthOptionsController extends ApiController {
  ...
  async request(@Body() data: RestaurantRequestChangePasswordDto) {
    try {
      await this.dispatch(
  ...
  async execute(@Body() data: RestaurantVerifyResetPasswordCodeDto) {
    try {
      return {
        valid: (
          await this.ask(
  ...
  async change(@Body() data: RestaurantChangePasswordDto) {
    try {
      await this.dispatch(
