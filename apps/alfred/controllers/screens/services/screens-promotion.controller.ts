...
import ScreensPromotionInfrastructureCommandRepository from '@screens/auth/infrastructure/persistance/typeorm/repositories/screens-promotion-infrastructure-command-repository';
import ListDto from '@apps/shared/dto/list-dto';

@ApiTags('Screens')
@Controller({
  path: 'screens/promotion',
})
export class ScreensPromotionController extends ApiController {
  constructor(
    @inject('command.bus')
    commandBus: CommandBus,
    @inject('query.bus')
    queryBus: QueryBus,
    @inject('ScreensPromotionRepository')
    private repo: ScreensPromotionInfrastructureCommandRepository,
