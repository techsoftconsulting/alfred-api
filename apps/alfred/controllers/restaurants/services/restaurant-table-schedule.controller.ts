...
import RestaurantTableScheduleInfrastructureCommandRepository from '@restaurants/auth/infrastructure/persistance/typeorm/repositories/restaurant-table-schedule-infrastructure-command-repository';
...
class RestaurantTableScheduleDto {}

@ApiTags('Restaurant')
@ApiBearerAuth()
@UseGuards(JwtUserGuard)
@Controller({
  path: 'restaurant/table-schedule',
})
export class RestaurantTableScheduleController extends ApiController {
  constructor(
    @inject('command.bus')
    commandBus: CommandBus,
    @inject('query.bus')
    queryBus: QueryBus,
    @inject('RestaurantTableScheduleRepository')
    private repo: RestaurantTableScheduleInfrastructureCommandRepository,
  ...
  async list(
    ...
    try {
      ...
      return await this.repo.findSchedules(criteria);
    } catch (error) {
      console.log(error);
  ...
  async create(@Body() data: RestaurantTableScheduleDto): Promise<any> {
    try {
      await this.repo.createSchedule(data);
  ...
  async update(@Body() data: RestaurantTableScheduleDto): Promise<any> {
    try {
      await this.repo.updateSchedule(data);
