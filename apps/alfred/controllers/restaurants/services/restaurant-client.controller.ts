...
import RestaurantClientInfrastructureCommandRepository from '@restaurants/auth/infrastructure/persistance/typeorm/repositories/restaurant-client-infrastructure-command-repository';
...
class RestaurantClientDto {}

@ApiTags('Restaurant')
@ApiBearerAuth()
@UseGuards(JwtUserGuard)
@Controller({
  path: 'restaurant/client',
})
export class RestaurantClientController extends ApiController {
  constructor(
    @inject('command.bus')
    commandBus: CommandBus,
    @inject('query.bus')
    queryBus: QueryBus,
    @inject('RestaurantClientRepository')
    private repo: RestaurantClientInfrastructureCommandRepository,
  ...
  async list(
    ...
    try {
      ...
      return await this.repo.findClients(criteria);
    } catch (error) {
      console.log(error);
  ...
  async create(@Body() data: RestaurantClientDto): Promise<any> {
    try {
      await this.repo.createClient(data);
  ...
  async update(@Body() data: RestaurantClientDto): Promise<any> {
    try {
      await this.repo.updateClient(data);
