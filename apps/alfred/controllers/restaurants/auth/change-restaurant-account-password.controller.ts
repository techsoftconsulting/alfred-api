...
class RestaurantAccountChangePasswordDto {
  @ApiProperty()
  password: string;
...
export class ChangeRestaurantAccountPasswordController extends ApiController {
  constructor(
    @inject('command.bus')
  ...
  @ApiResponse({
    status: 200,
    description: 'Restaurant change password',
  })
  @ApiOperation({
  ...
  async execute(
    ...
    @Body() data: RestaurantAccountChangePasswordDto,
  ) {
    try {
      const token = await this.dispatch(
        new AuthenticateRestaurantAccountCommand(user.email, data.oldPassword),
      );
